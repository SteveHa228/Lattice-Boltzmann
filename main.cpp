...
#include "LTexture.h"
#include "LatticeBoltzmannCL.h"
// problem parameters

const int     N = 128;//128;                  // number of node points along X and Y (cavity length in lattice units)
...
//          scalar[i*WIDTH + j] = pow((ux_interp*ux_interp + uy_interp*uy_interp), 0.5) / LID_VELOCITY;   // normalized velocity magnitude
            ...
            */
            ...
            //glVertex3f(x_actual + dx/2, y_actual + dy/2, 0.0);
        }
    }
    //glEnd();
    // free memory
...
int main(int argc, char* argv[])
{
    ...
    initializeCL(N, Q, DENSITY, LID_VELOCITY, ex, ey, oppos, wt, rho, ux, uy, sigma, f, feq, f_new);
    ...
    //Load texture
    if( !gLoadedTexture.loadTextureFromFile( "barriers/barrier1.png" ) )
    {
        printf( "Unable to load file texture!\n" );
        return false;
    }
    printf("width  = %d, height = %d\n", gLoadedTexture.textureWidth(), gLoadedTexture.textureHeight());
    textureBarrier = gLoadedTexture.getTextureID();
    glBindTexture(GL_TEXTURE_2D, textureBarrier);
    GLubyte* pixels = new GLubyte[gLoadedTexture.textureWidth() * gLoadedTexture.textureHeight() * sizeof(GLubyte)];
    glGetTexImage(GL_TEXTURE_2D, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, pixels);
    data = reinterpret_cast<GLuint*>(pixels);
